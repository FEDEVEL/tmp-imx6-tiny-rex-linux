...
static int mxc_free_frame_buf(cam_data *cam)
{
	...
	pr_err("MVC: In mxc_free_frame_buf\n");
...
static int mxc_allocate_frame_buf(cam_data *cam, int count)
{
	...
	pr_err("In MVC:mxc_allocate_frame_buf - size=%d\n",
...
{
	pr_err("In MVC:mxc_v4l2_buffer_status\n");
...
{
	pr_err("In MVC:mxc_v4l2_release_bufs\n");
	return 0;
}

static int mxc_v4l2_prepare_bufs(cam_data *cam, struct v4l2_buffer *buf)
{
	pr_err("In MVC:mxc_v4l2_prepare_bufs\n");
...
static int mxc_streamon(cam_data *cam)
{
	...
	pr_err("*************** START mxc_streamon pixelformata = %d\n",cam->v2f.fmt.pix.pixelformat);
	...
	pr_err("*************** END mxc_streamon pixelformata = %d\n",cam->v2f.fmt.pix.pixelformat);
	...
	do {
		...
		if (((strncmp(fbi->fix.id, "DISP3 BG", 8) == 0) &&
			...
			if (ipu_ch == MEM_BG_SYNC) {
				bg_fbi = fbi;
				pr_err("Found background frame buffer.\n");
			}
		}
...
static int mxc_v4l2_g_fmt(cam_data *cam, struct v4l2_format *f)
{
	...
	pr_err("In MVC: mxc_v4l2_g_fmt type=%d\n", f->type);

	switch (f->type) {
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		pr_err("   type is V4L2_BUF_TYPE_VIDEO_CAPTURE\n");
		f->fmt.pix = cam->v2f.fmt.pix;
		break;
	case V4L2_BUF_TYPE_VIDEO_OVERLAY:
		pr_err("   type is V4L2_BUF_TYPE_VIDEO_OVERLAY\n");
		f->fmt.win = cam->win;
		break;
	default:
		pr_err("   type is invalid\n");
		retval = -EINVAL;
	}
	...
	pr_err("End of %s: crop_current widthxheight %d x %d\n",
		 __func__,
		 cam->crop_current.width, cam->crop_current.height);
	pr_err("End of %s: cam->v2f.fmt.pix = %d\n",
		 __func__,
		 cam->v2f.fmt.pix.pixelformat);
...
static int mxc_v4l2_s_fmt(cam_data *cam, struct v4l2_format *f)
{
	...
	if (cam->v2f.fmt.pix.pixelformat == V4L2_PIX_FMT_YUV420){
		pr_err("FOUND: IPU_PIX_FMT_YUV420P\n");
	} else if (cam->v2f.fmt.pix.pixelformat == V4L2_PIX_FMT_YVU420){
		pr_err("FOUND: IPU_PIX_FMT_YVU420P\n");
	} else if (cam->v2f.fmt.pix.pixelformat == V4L2_PIX_FMT_YUV422P){
		pr_err("FOUND: IPU_PIX_FMT_YUV422P\n");
	} else if (cam->v2f.fmt.pix.pixelformat == V4L2_PIX_FMT_UYVY){
		pr_err("FOUND: IPU_PIX_FMT_UYVY\n");
	} else if (cam->v2f.fmt.pix.pixelformat == V4L2_PIX_FMT_YUYV){
		pr_err("FOUND: IPU_PIX_FMT_YUYV\n");
	} else if (cam->v2f.fmt.pix.pixelformat == V4L2_PIX_FMT_NV12){
		pr_err("FOUND: IPU_PIX_FMT_NV12\n");
	} else if (cam->v2f.fmt.pix.pixelformat == V4L2_PIX_FMT_BGR24){
		pr_err("FOUND: IPU_PIX_FMT_BGR24\n");
	} else if (cam->v2f.fmt.pix.pixelformat == V4L2_PIX_FMT_RGB24){
		pr_err("FOUND: IPU_PIX_FMT_RGB24\n");
	} else if (cam->v2f.fmt.pix.pixelformat == V4L2_PIX_FMT_RGB565){
		pr_err("FOUND: IPU_PIX_FMT_RGB565\n");
	} else if (cam->v2f.fmt.pix.pixelformat == V4L2_PIX_FMT_BGR32){
		pr_err("FOUND: IPU_PIX_FMT_BGR32\n");
	} else if (cam->v2f.fmt.pix.pixelformat == V4L2_PIX_FMT_RGB32){
		pr_err("FOUND: IPU_PIX_FMT_RGB32\n");
	} else {
...
static int mxc_v4l2_s_param(cam_data *cam, struct v4l2_streamparm *parm)
{
	...
	pr_err("   Current capabilities are %x\n",
			currentparm.parm.capture.capability);
	pr_err("   Current capturemode is %d  change to %d\n",
			currentparm.parm.capture.capturemode,
			parm->parm.capture.capturemode);
	pr_err("   Current framerate is %d  change to %d\n",
	...
	pr_err("   g_fmt_cap returns widthxheight of input as %d x %d\n",
	...
	pr_err("*************** cam_fmt.fmt.pix.pixelformata after vidioc_int_s_parm pixelformata = ");

	if (cam_fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_YUV420){
		pr_err("FOUND: IPU_PIX_FMT_YUV420P\n");
	} else if (cam_fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_YVU420){
		pr_err("FOUND: IPU_PIX_FMT_YVU420P\n");
	} else if (cam_fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_YUV422P){
		pr_err("FOUND: IPU_PIX_FMT_YUV422P\n");
	} else if (cam_fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_UYVY){
		pr_err("FOUND: IPU_PIX_FMT_UYVY\n");
	} else if (cam_fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_YUYV){
		pr_err("FOUND: IPU_PIX_FMT_YUYV\n");
	} else if (cam_fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_NV12){
		pr_err("FOUND: IPU_PIX_FMT_NV12\n");
	} else if (cam_fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_BGR24){
		pr_err("FOUND: IPU_PIX_FMT_BGR24\n");
	} else if (cam_fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_RGB24){
		pr_err("FOUND: IPU_PIX_FMT_RGB24\n");
	} else if (cam_fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_RGB565){
		pr_err("FOUND: IPU_PIX_FMT_RGB565\n");
	} else if (cam_fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_BGR32){
		pr_err("FOUND: IPU_PIX_FMT_BGR32\n");
	} else if (cam_fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_RGB32){
		pr_err("FOUND: IPU_PIX_FMT_RGB32\n");
	} else {
		pr_err("format not supported\n");
		return -EINVAL;
...
{
	pr_err("In mxc_v4l2_s_std %Lx\n", e);

	if (e == V4L2_STD_PAL) {
		pr_err("   Setting standard to PAL %Lx\n", V4L2_STD_PAL);
		cam->standard.id = V4L2_STD_PAL;
		video_index = TV_PAL;
	} else if (e == V4L2_STD_NTSC) {
		pr_err("   Setting standard to NTSC %Lx\n",
				V4L2_STD_NTSC);
...
static int mxc_v4l_open(struct file *file)
{
	...
	if (cam->open_count++ == 0) {
		...
		csi_param.data_fmt = cam_fmt.fmt.pix.pixelformat;
		cam->v2f.fmt.pix.pixelformat = cam_fmt.fmt.pix.pixelformat; //!rf
		...
		vidioc_int_dev_init(cam->sensor);
		pr_err("********* Camera Open *********** Name = %s Format: Set = %d / Saved = %d\n",mxc_capture_inputs[cam->current_input].name,cam_fmt.fmt.pix.pixelformat,cam->v2f.fmt.pix.pixelformat);
	}
...
static int mxc_v4l_close(struct file *file)
{
	...
	if (--cam->open_count == 0) {
		...
		wait_event_interruptible(cam->power_queue,
					 cam->low_power == false);
		pr_err("mxc_v4l_close: release resource\n");
			    ...
			    unsigned int ioctlnr, void *arg)
{
	...
	pr_err("In MVC: mxc_v4l_do_ioctl %x\n", ioctlnr);
	pr_err("**file** = %lu\n",file_inode(file)->i_ino);
	...
	struct v4l2_format *tmparg = arg;
	if (tmparg->fmt.pix.pixelformat == V4L2_PIX_FMT_YUV420){
		pr_err("ARG: IPU_PIX_FMT_YUV420P\n");
	} else if (tmparg->fmt.pix.pixelformat == V4L2_PIX_FMT_YVU420){
		pr_err("ARG: IPU_PIX_FMT_YVU420P\n");
	} else if (tmparg->fmt.pix.pixelformat == V4L2_PIX_FMT_YUV422P){
		pr_err("ARG: IPU_PIX_FMT_YUV422P\n");
	} else if (tmparg->fmt.pix.pixelformat == V4L2_PIX_FMT_UYVY){
		pr_err("ARG: IPU_PIX_FMT_UYVY\n");
	} else if (tmparg->fmt.pix.pixelformat == V4L2_PIX_FMT_YUYV){
		pr_err("ARG: IPU_PIX_FMT_YUYV\n");
	} else if (tmparg->fmt.pix.pixelformat == V4L2_PIX_FMT_NV12){
		pr_err("ARG: IPU_PIX_FMT_NV12\n");
	} else if (tmparg->fmt.pix.pixelformat == V4L2_PIX_FMT_BGR24){
		pr_err("ARG: IPU_PIX_FMT_BGR24\n");
	} else if (tmparg->fmt.pix.pixelformat == V4L2_PIX_FMT_RGB24){
		pr_err("ARG: IPU_PIX_FMT_RGB24\n");
	} else if (tmparg->fmt.pix.pixelformat == V4L2_PIX_FMT_RGB565){
		pr_err("ARG: IPU_PIX_FMT_RGB565\n");
	} else if (tmparg->fmt.pix.pixelformat == V4L2_PIX_FMT_BGR32){
		pr_err("ARG: IPU_PIX_FMT_BGR32\n");
	} else if (tmparg->fmt.pix.pixelformat == V4L2_PIX_FMT_RGB32){
		pr_err("ARG: IPU_PIX_FMT_RGB32\n");
	} else {
		pr_err("format not supported\n");
	}
	...
	case VIDIOC_S_CROP: {
		...
		pr_err("   Cropping Input to ipu size %d x %d\n",
	...
	default:
		pr_err("   case default or not supported\n");
		retval = -EINVAL;
		break;
	}
...
static int init_camera_struct(cam_data *cam, struct platform_device *pdev)
{
	...
	pr_err("In MVC: init_camera_struct\n");
...
static int mxc_v4l2_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	pr_err("In MVC:mxc_v4l2_suspend\n");
...
static int mxc_v4l2_master_attach(struct v4l2_int_device *slave)
{
	...
	pr_err("In MVC: mxc_v4l2_master_attach\n");
	pr_err("   slave.name = %s\n", slave->name);
	pr_err("   master.name = %s\n", slave->u.slave->master->name);
...
static void mxc_v4l2_master_detach(struct v4l2_int_device *slave)
{
	...
	pr_err("In MVC:mxc_v4l2_master_detach\n");
